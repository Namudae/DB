...
--블록 잡고 Ctrl + Enter
alter user hr account unlock; --계정 잠금해제 명령어
...
-- *** 스크립트 파일은 계정에 비종속적이다. 
-- 스크립트 파일은 계정에 독립적이다.
-- *** 스크립트의 내용을 실행할 당시의 접속 상태가 누구인지가 중요하다. 
...
SELECT * FROM tabs; -- FM. Ctrl + F7 누르면 자동 변환

-- 모든 구문을 대문자로 작성하는 것을 권장한다.
